...
export function view2dot(view, stamp?) {
  ...
  keys.forEach(key => {
    ...
    if (op._targets) op._targets.forEach(t => {
      ...
      if (argop(t, op) === 'query') {
        ids[op.id]['query'] = ids[op.id].value.value
      }
    });
  });
...
function tooltip(node) {
  if (node.query) {
    return ' [tooltip="' + escapeRegExp(node.query) + '"]'
  }
  return ''
...
export function nodeFillColor(node, stamp?) {
...
export function nodeColor(node, stamp) {
