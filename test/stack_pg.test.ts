...
describe("simple stack transform example", () => {
  test("stack", async () => {
    var spec_vg = require(`../sample_data/specs/vega_specs/stack.json`);
...
describe.each(test_cases)('stack transform with table product', (name, transform) => {
  test(`${name}`, async () => {
    var spec_vg = require('../sample_data/specs/vega_specs/product.json');
    ...
    var spec = require('../sample_data/specs/specs/product.json');
    const dbtransform = {
      "type": "dbtransform",
      "relation": "product"
    }
